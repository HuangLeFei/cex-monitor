            ...
            while let Some(msg) = read.next().await {
                match msg {
                    Ok(Message::Text(text)) => {
                        ...
                        // 处理公告
                        if v.get("channel")
                            == Some(&Value::String("announcement.summary_listing".into()))
                        {
                            if let Some(result) = v.get("result") {
                                if let Some(published) =
                                    result.get("published_at").and_then(|v| v.as_u64())
                                {
                                    unsafe {
                                        // 检查新公告
                                        if LAST_GATE_PUBLISHED
                                            .map_or(true, |prev| prev != published)
                                        {
                                            LAST_GATE_PUBLISHED = Some(published);
                                            let title = result
                                                .get("title")
                                                .and_then(|v| v.as_str())
                                                .unwrap_or("（Gate公告 无标题）");
                                            println!("🆕 Gate 上币公告:\n📄 {}", title);
                                            if let Err(e) =
                                                tg::send_to_tg("Gate", title, None).await
                                            {
                                                eprintln!("❌ 发送到TG失败: {}", e);
                                            }
                                        }
